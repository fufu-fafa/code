...
// config
const std::size_t MAX_TRAIL_LEN = 25;
const float WIDTH = 800.f;
const float HEIGHT = 600.f;
const int AMOUNT = 32;
const int RAD = 15.f;
const float CSPEED = 240.f;
...
int main() {
    ...
    for (int n = 0; n < AMOUNT; n++) {
        ...
        circles[n].trail.clear();
        circles[n].trail.push_back(circles[n].pos);
    }
    ...
    ghost.setOrigin(sf::Vector2f(RAD, RAD));
    while (window.isOpen()) {
        ...
        for (int n = 0; n < AMOUNT; n++) {
            ...
            circles[n].trail.push_back(circles[n].pos);
            if (circles[n].trail.size() > MAX_TRAIL_LEN) circles[n].trail.pop_front();
            ghost.setRadius(circles[n].rad);
            ghost.setOrigin(sf::Vector2f(circles[n].rad, circles[n].rad));
            const std::size_t len = circles[n].trail.size();
            if (len > 0) {
                for (std::size_t i = 0; i < len; ++i) {
                    float tNormalized = static_cast<float>(i + 1) / static_cast<float>(len);
                    uint8_t alpha = static_cast<uint8_t>(tNormalized * 255.f);
                    sf::Color base = circles[n].shape.getFillColor();
                    sf::Color ghostColor(base.r, base.g, base.b, alpha);
                    ghost.setFillColor(ghostColor);
                    ghost.setPosition(circles[n].trail[i]);
                    window.draw(ghost);
                }
            }
